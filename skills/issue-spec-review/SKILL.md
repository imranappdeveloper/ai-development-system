...
## Script-first (mandatory)

Run structural checks **before** any AI judgment:

```bash
"$AI_DEV_OS_HOME/scripts/issue-spec-check.sh" --issue <N>
```

| Script output | Agent action |
|---|---|
| `NEEDS_SPEC` | Stop — comment on issue, label `needs-info`, remove `ready-for-agent`. Do **not** run semantic review. |
| `afk-preflight-skip: true` | Skip this skill entirely (stamp + hash validated by script). |
| `READY` | Continue with **semantic checklist only** below. |

Structural checks (script-owned): sections present, ≥2 checkbox ACs, `Blocked by` refs, `spec-sha256`.

## When to run

- **Mandatory:** `/work-to-pr-v2` per-issue loop, before `in-progress` label — unless script reports `afk-preflight-skip: true`
- **Skipped:** when script reports `afk-preflight-skip: true` (replaces manual stamp/hash check)
- **Optional:** manually on any issue: `/issue-spec-review 42`

Legacy stamps without `spec-sha256` → run script + full semantic review once.

## Input

Fetch the full issue via `gh issue view <N> --json title,body,labels` when not already loaded for the script.
...
## Semantic checklist (AI-owned — after structural READY)
...
Items 1, 2, 4, 6 are **script-owned** — do not re-check manually unless the script failed.
