...
## Local codebase survey (optional — Mac interactive)

Read-only discovery via local Ollama when a **script or artifact already lists files**. Full spec: **`$AI_DEV_OS_HOME/docs/LOCAL-SURVEY.md`**.

| When | Action |
|------|--------|
| File list from `grill-intake.py`, `issue-context-pack.sh`, `work/requirement-lock.md`, or user `@file` | Call MCP tool **`survey`** (`codebase-survey`) with that list |
| No file list yet | Run script first, or use **cloud explore** — never free-roam local |
| `ok: false` or `fallback_recommended: true` in survey JSON | **Fail-closed:** cloud explore on the **same file list only** |
| Implementation, TDD, PR, AFK `task-run` | **Cloud only** — no local survey |
...
6. Grill → `work/requirement-lock.md` → overview → `/plan-to-issue-v2`; Start AFK via `task-run-server.sh`
