...
**Draft UI/UX** — Plan-phase optional playbook producing the UI/UX Plan artifact (UIUX).

| Field | Value |
|-------|-------|
| skill_id | `PB-draft-ui-ux` |
| display_name | UI/UX Planner |
| version | `1.0.0` |
| prompt_version | `1.0.0` |
| status | `draft` |
| exit_gate | `H-PLAN` (human) |
| checklist | `CL-UIUX` |
| template | `templates/ui-ux/template.md` (TP-uiux) |
| path | `{AI_DEV_OS_HOME}/playbooks/draft-ui-ux/` |
...
Invoke  → approved PRD + work_id + project_root + AI_DEV_OS_HOME
          ARCH + DISC linked or arch_gap / disc_gap waiver (soft)
Require → PRD artifact; H-PLAN on PRD (or waiver)
Produce → UIUX + updated Work Record + Validation + Handoff
Gate    → CL-UIUX (agent) then H-PLAN (human)
...
|-------------|---------------------|
| WF-FEATURE | new |
| WF-ENHANCEMENT | additive \| redesign |
...
| [06-quality.md](./06-quality.md) | ACs & CL-UIUX map |
...
| Path | Purpose |
|------|---------|
| [examples/](./examples/) | Golden UIUX + anti-patterns |
...
| prerequisite | PB-draft-api `test-runs/latest-gate.md` VERDICT PASS |