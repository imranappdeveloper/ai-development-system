...
**Performance Baseline** — Plan-phase playbook producing **PERF-BASE** artifacts: targets, SLOs, and measurement plans. **Does not execute load tests.**

| Field | Value |
|-------|-------|
| skill_id | `PB-perf-baseline` |
| display_name | Performance Baseline |
| version | `1.0.0` |
| prompt_version | `1.0.0` |
| status | `active` |
| exit_gate | `H-PLAN` (human) |
| checklist | `CL-PERF` |
| path | `{AI_DEV_OS_HOME}/playbooks/perf-baseline/` |
...
Produce → PERF-BASE at work/performance/{work_id}.md
Gate    → CL-PERF (agent) then H-PLAN (human)
Stop    → await H-PLAN; recommend PB-implement or PB-perf-review
```
...
| workflow_id | Notes |
|-------------|-------|
| WF-PERF | Primary path — baseline before implement |
...
| [05-context.md](./05-context.md) | Context budget (STD-PERF-001) |
...
| [fixtures/](./fixtures/) | wf-perf-alpha project workspace |
...
- Verify-phase review: `PB-perf-review` (static CODE vs PERF-BASE)
- Standard: `standards/engineering/STD-PERF-001.md`
- Orchestrator SSOT: `workflows/project-orchestrator/routing-matrix.yaml` → `skills.PB-perf-baseline`