...
| T1 | ISS/ISS-* full text, API (soft), DB (soft), WR frontmatter, INDEX workflow row | ≤15% session |
| T2 | CONTEXT.md: stack, module map, test conventions | ≤10% session |
| T3 | Bounded code reads (route headers, migration filenames, test file names) | ≤25% session |
...
| `{project_root}/work/api/{work_id}.md` | API contract grounding (soft) |
| `{project_root}/work/database/{work_id}.md` | DB entity/migration grounding (soft) |
| `{project_root}/work/architecture/{work_id}.md` | Component boundaries (optional) |
| `{project_root}/work/{work_id}.md` | Work Record |
| `{project_root}/CONTEXT.md` | Stack, test runner, module layout |
| `{project_root}/**/routes/**` | Route registration markers |
| `{project_root}/**/migrations/**` | Migration index — filenames only |
| `{project_root}/**/tests/**` | Test file markers — not full suite dump |
| `{project_root}/openapi.yaml` | Index only when generating spec |
...
| API body | ≤5 KB effective | Extract operation IDs; cite API path |
| DB body | ≤5 KB effective | Extract entity names; cite DB path |
...
| Session | Cache INDEX + CL-IMPLEMENT-BACKEND for run duration only |
