...
**Draft Database** — Plan-phase optional playbook producing the Database Design artifact (DB).
...
| checklist | `CL-DATABASE` |
| template | `templates/database/template.md` (TP-database) |
| path | `{AI_DEV_OS_HOME}/playbooks/draft-database/` |
...
Invoke  → approved ARCH + work_id + project_root + AI_DEV_OS_HOME
          PRD linked or prd_gap waiver (soft)
Require → ARCH artifact; H-PLAN on ARCH (or waiver)
Produce → DB + updated Work Record + Validation + Handoff
Gate    → CL-DATABASE (agent) then H-PLAN (human)
...
|-------------|---------------------|
| WF-FEATURE | new_schema |
| WF-REFACTOR | migration |
| WF-SECURITY | migration \| optimization |
| WF-PERF | optimization |
...
| [06-quality.md](./06-quality.md) | ACs & CL-DATABASE map |
| [07-edge-cases.md](./07-edge-cases.md) | P0 edge cases (26 EC-*) |
| [08-limitations.md](./08-limitations.md) | Boundaries |
| [09-system-prompt.md](./09-system-prompt.md) | Deployable prompt |
| [10-review.md](./10-review.md) | Architect review (73/100) |
...
| Path | Purpose |
|------|---------|
| [examples/](./examples/) | Golden DB + anti-patterns |
