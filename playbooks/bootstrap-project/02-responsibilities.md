# PB-bootstrap-project — Responsibilities
...
| P3 | Set `scaffold_scope` from PRD surface and ARCH hints | Enums set per registry.yaml |
| P4 | Draft SCAFFOLD body per 04-io-contract | All required sections present |
| P5 | Trace upstream references | INT/PRD/DIAG paths in References block |
| P6 | Persist OUT-01 artifact | `work/scaffold/{work_id}.md` written or `persist: pending` |
| P7 | Update Work Record | Link SCAFFOLD; status `scaffold_pending_review` |
| P8 | Run CL-BOOTST | Validation record = pass |
| P9 | Prepare handoff for H-PLAN | `decision: pending` only |
| P10 | Recommend next skill (non-binding) | `PB-onboard-project` when criteria met |
...
| S2 | Flag confidence gaps | `scaffold_confidence: low` with blockers list |
...
| N9 | Store decisions only in chat | Must persist SCAFFOLD |
| N10 | Self-approve artifact | Human at H-PLAN |
| N11 | Skip CL-BOOTST | Never |
...
| Task | Agent | Human |
|------|-------|-------|
| Draft SCAFFOLD | Yes | Revise notes |
...
| Type | ID | Rule |
|------|-----|------|
| Skill | PB-draft-prd | Upstream producer |
| Artifact | SCAFFOLD path | OUT-01 destination |
| Checklist | CL-BOOTST | Blocks handoff on fail |
