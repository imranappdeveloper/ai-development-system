...
| Step | ID | Action |
|------|-----|--------|
| 1 | INIT | Verify entry criteria; load INDEX, CL-BOOTST, upstream path from WR |
| 2 | LOAD | Read upstream + soft artifacts + CONTEXT slice |
| 3 | ANALYZE | Set `scaffold_scope` from PRD surface and ARCH hints |
| 4 | DOC | Build SCAFFOLD per 04-io-contract |
| 5 | PERSIST | Write `{project_root}/work/scaffold/{work_id}.md`; update Work Record |
| 6 | VAL | CL-BOOTST (10 checks); recovery ≤3 attempts |
...
| EC-03 | No prior SCAFFOLD with H-PLAN `approve` unless `mode: revise` |
...
| XC-02 | CL-BOOTST `result: pass` |
| XC-03 | OUT-04 handoff includes `gate_id: H-PLAN`, `decision: pending` |
| XC-04 | WR `status: scaffold_pending_review` |
...
| On approve | WR status advanced; may recommend `PB-onboard-project` |
...
Human `revise` at H-PLAN → re-enter **LOAD** → update SCAFFOLD → full CL-BOOTST → handoff again.

---

## Recovery

CL-BOOTST fail → fix per `checklists/bootstrap.md` recovery table → re-VAL (≤3) → OUT-05 escalation.
...
|--------|---------|-----------|
| Default success | PB-onboard-project | Per routing-matrix |
...
|--------------|-------------|------------------|
| PB-draft-prd | `playbooks/draft-prd/test-runs/latest-gate.md` | VERDICT PASS |
