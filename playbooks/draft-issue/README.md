...
**Bugfix Issue Draft** — Plan-phase playbook producing ISS artifacts.

| Field | Value |
|-------|-------|
| skill_id | `PB-draft-issue` |
| display_name | Bugfix Issue Draft |
| version | `1.0.0` |
| prompt_version | `1.0.0` |
| status | `active` |
| exit_gate | `H-PLAN` (human) |
| checklist | `CL-ISSUE` |
| path | `{AI_DEV_OS_HOME}/playbooks/draft-issue/` |
...
Produce → ISS at work/issue/{work_id}.md
Gate    → CL-ISSUE (agent) then H-PLAN (human)
Stop    → await H-PLAN; recommend PB-implement
```
