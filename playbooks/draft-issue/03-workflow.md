...
| Step | ID | Action |
|------|-----|--------|
| 1 | INIT | Verify entry criteria; load INDEX, CL-ISSUE, upstream path from WR |
| 2 | LOAD | Read upstream + soft artifacts + CONTEXT slice |
| 3 | ANALYZE | Map repro + root cause to testable fix ACs; set `issue_lane` |
| 4 | DOC | Build ISS per 04-io-contract |
| 5 | PERSIST | Write `{project_root}/work/issue/{work_id}.md`; update Work Record |
| 6 | VAL | CL-ISSUE (10 checks); recovery ≤3 attempts |
...
| EC-03 | No prior ISS with H-PLAN `approve` unless `mode: revise` |
...
Human `revise` at H-PLAN → re-enter **LOAD** → update ISS → full CL-ISSUE → handoff again.

---

## Recovery

CL-ISSUE fail → fix per `checklists/issue.md` recovery table → re-VAL (≤3) → OUT-05 escalation.
...
|--------|---------|-----------|
| Default success | PB-implement | Per routing-matrix |
...
Optional upstream PB-diagnose-bug may run first; ISS must link DIAG when present per CL-ISSUE #4.
